...
using SalonHub.Web.Startup;
using AngleSharp.Html.Dom;
using AngleSharp.Html.Parser;
using Microsoft.AspNetCore.Hosting;
using Shouldly;

namespace SalonHub.Web.Tests
{
    public abstract class SalonHubWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static SalonHubWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(SalonHubWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<SalonHubDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<SalonHubDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<SalonHubDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<SalonHubDbContext, Task<T>> func)
        {
            T result;
